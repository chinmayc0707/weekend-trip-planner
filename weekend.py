    ...
    try:
        ...
        for chunk in response:
            try:
                ...
                # The text property raises a ValueError on chunks that only contain metadata (like the last one)
                pass
    except Exception as e:
        yield f"\n\nError generating route: {str(e)}"
...
def generate_sse(inputs, api_key=None):
    """Wraps the trip plan generator in SSE (Server-Sent Events) format.
    Reverse proxies recognize text/event-stream and do NOT buffer it."""
    try:
        for chunk in generate_trip_plan(inputs, api_key):
...
def stream():
    ...
    response = Response(
        stream_with_context(generate_sse(inputs, api_key)),
